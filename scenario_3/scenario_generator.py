...
def control_host(host_vehicle):
    ...
    while True:
        ...
        if normal_drive:
            throttle, last_error_pdcc, error_sum_pdcc= pd_control_vel(velocity_, config.host_target_vel_3,
        ...
        if emergency_control:
            ...
            ## strategy-2 : pd control
                ...
                val, last_error_pdcs, error_sum_pdcs = pd_control_for_safety(score[2], 0.8, last_error_pdcs,
                                                                             error_sum_pdcs, kp=1., kd=0.1, ki=0.)
                # print('brake -- ', val)
                if val >= 0:
                    print('brake -- ', val)
            ...
            ## strategy-3 : pd control
                ...
                val, last_error_pdcs, error_sum_pdcs = pd_control_for_safety(min(score[0] + score[1], 1.), 0.5, last_error_pdcs,
...
def control_other(other_vehicle):
    ...
    def pd_control_steer(pos_longitd, target_pos_longitd, last_error, error_sum):
        ...
        steer = k_p * error + k_d * d_error + k_i * error_sum

        return max(min(steer, 1.), -1.), error, error_sum
    ...
    while True:
        ...
        host_vehicle_pos = host_vehicle.get_location()
        host_vehicle_pos_x = host_vehicle_pos.x

        other_vehicle_pos = other_vehicle.get_location()
        other_vehicle_pos_x = other_vehicle_pos.x
        other_vehicle_pos_y = other_vehicle_pos.y
        ...
        # throttle, last_error, error_sum = pd_control_vel(other_vehicle_vel_, config.other_vehicle_vel_3, last_error, error_sum)
        if turn_flag:
            throttle, last_error, error_sum = pd_control_vel(other_vehicle_vel_, config.host_target_vel_3+1., last_error,
                                                             error_sum)
            steer, last_error_s, error_sum_s = pd_control_steer(other_vehicle_pos_y, 204.5, last_error_s, error_sum_s)
            # print('steer -- ', steer)
            control = carla.VehicleControl(throttle=throttle, steer=steer)
        else:
            throttle, last_error, error_sum = pd_control_vel(other_vehicle_vel_, config.other_vehicle_vel_3, last_error,
                                                             error_sum)
            control = carla.VehicleControl(throttle=throttle)
...
if __name__ == '__main__':
    ...
    transform = carla.Transform(carla.Location(x=config.host_vehicle_init_pos_3[0],
                                               y=config.host_vehicle_init_pos_3[1], z=1.8),
    ...
    transform = carla.Transform(carla.Location(x=config.other_vehicle_init_pos_3[0],
                                               y=config.other_vehicle_init_pos_3[1], z=1.8),
                                carla.Rotation(pitch=0., yaw=0., roll=0.))
